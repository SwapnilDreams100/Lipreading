...
#         else:
        ...
        return preprocess_data, label, self.list[idx][0]
...
def pad_packed_collate(batch):
    if len(batch) == 1:
        data, lengths, labels_np, filename= zip(*[(a, a.shape[0], b, c) for (a, b, c) in sorted(batch, key=lambda x: x[0].shape[0], reverse=True)])
        data = torch.FloatTensor(np.array(data))
        lengths = [data.size(1)]
        return data, lengths, torch.LongTensor(labels_np), filename
    ...
    if len(batch) > 1:
        ...
        if data_list[0].ndim == 4:
            max_len, h, w, c = data_list[0].shape  # since it is sorted, the longest video is the first one
            data_np = np.zeros(( len(data_list), max_len, h, w, c))
