...
__all__ = ['Compose', 'Normalize', 'CenterCrop', 'RgbToGray', 'RandomCrop','Resize',
...
class Resize(object):
    ...
    def __call__(self, frames):
        t, h, w = frames.shape
        res = F.interpolate(torch.tensor(frames).unsqueeze(1),[self.size, self.size], mode= 'bilinear')
        return torch.squeeze(res,1).numpy()
